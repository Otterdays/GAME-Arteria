/**
 * Herblore Screen — Brew herbs + vials into potions.
...
import { HERBLORE_RECIPES, HerbloreRecipe } from '@/constants/herblore';
...
function canAffordRecipe(inventory: { id: string; quantity: number }[], recipe: HerbloreRecipe): boolean {
...
function minBatchesAffordable(inventory: { id: string; quantity: number }[], recipe: HerbloreRecipe): number {
    let min = Infinity;
    for (const consumed of recipe.consumedItems) {
        const owned = inventory.find((i) => i.id === consumed.id)?.quantity ?? 0;
...
export default function HerbloreScreen() {
    useIdleSoundscape('herblore');
    ...
    const herbSkill = player.skills.herblore;
    const inventory = player.inventory;
    const activeTask = player.activeTask;

    const isBrewing = activeTask?.skillId === 'herblore';
    const activeRecipeId = isBrewing ? activeTask.actionId : null;
    const activeRecipe = HERBLORE_RECIPES.find((r) => r.id === activeRecipeId);

    const [popTrigger, setPopTrigger] = React.useState(0);
    const lastXp = React.useRef(herbSkill.xp);
    const lastGain = React.useRef(0);
    const glowAnim = React.useRef(new Animated.Value(0)).current;

    React.useEffect(() => {
        if (herbSkill.xp > lastXp.current) {
            const gain = herbSkill.xp - lastXp.current;
            lastXp.current = herbSkill.xp;
    ...
    const styles = useMemo(
        () =>
            StyleSheet.create({
                ...
                levelTag: {
                    backgroundColor: `${palette.skillHerblore || herbColor}25`,
                    paddingHorizontal: Spacing.sm,
                    paddingVertical: 2,
                    borderRadius: Radius.full,
                    borderWidth: 1,
                    borderColor: `${palette.skillHerblore || herbColor}50`,
                },
                levelTagText: {
                    color: palette.skillHerblore || herbColor,
                    fontSize: FontSize.xs,
                    fontWeight: 'bold',
                    textTransform: 'uppercase',
                },
                ...getLevelBadgeStyles(palette, herbColor),
                titleRow: {
                    flexDirection: 'row',
                    alignItems: 'center',
    ...
    const clvXP = xpForLevel(herbSkill.level);
    const nlvXP = xpForLevel(herbSkill.level + 1);
    const xpIntoLevel = Math.max(0, Math.floor(herbSkill.xp - clvXP));
    const xpNeeded = Math.max(1, nlvXP - clvXP);
    const pct = herbSkill.level >= 99 ? 100 : Math.min(100, (xpIntoLevel / xpNeeded) * 100);

    const handleRecipePress = (recipe: HerbloreRecipe) => {
        if (herbSkill.level < recipe.levelReq) {
            showFeedbackToast({
                type: 'locked',
                title: 'Locked',
                message: `Requires Herblore Level ${recipe.levelReq}`,
            });
            return;
        }
        if (!canAffordRecipe(inventory, recipe)) {
            showFeedbackToast({
                type: 'warning',
                title: 'No Materials',
                message: 'You need herbs and empty vials. Harvest herbs and buy vials from Nick.',
            });
            return;
        }
    ...
    return (
        <Animated.View style={[styles.container, { paddingTop: insets.top }]}>
            <Animated.View
                style={[StyleSheet.absoluteFill, { backgroundColor: herbColor, opacity: glowOpacity, zIndex: 10 }]}
                pointerEvents="none"
            />
            <Stack.Screen options={{ title: 'Herblore', headerShown: false }} />
            ...
            <View style={styles.infoSection}>
                <View style={styles.titleRow}>
                    <TouchableOpacity
                        onPress={() => {
                            ...
                            router.replace(`/skills/${getPrevSkill('herblore')}`);
                        }}
                    ...
                    <View style={styles.titleContent}>
                        ...
                        <View style={styles.levelTag}>
                            <Text style={styles.levelTagText}>Lv. {herbSkill.level}</Text>
                        </View>
                    </View>
                    ...
                    <TouchableOpacity
                        onPress={() => {
                            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                            router.replace(`/skills/${getNextSkill('herblore')}`);
                        }}
                        style={styles.navButton}
                    >
                ...
                <View style={styles.xpRow}>
                    <View style={styles.xpBarBg}>
                        <ProgressBarWithPulse progress={pct} fillColor={herbColor} widthPercent={pct} />
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                        <Text style={styles.xpText}>
                            {herbSkill.level >= 99 ? '' : <AnimatedNumber value={xpIntoLevel} formatValue={(v) => formatNumber(v)} />}
                            {herbSkill.level >= 99 ? `${formatNumber(herbSkill.xp)} XP — MAX` : ` / ${formatNumber(xpNeeded)} XP`}
                        </Text>
                    </View>
            ...
            <ScrollView contentContainerStyle={styles.listContent}>
                {HERBLORE_RECIPES.map((recipe) => {
                    const isLevelLocked = herbSkill.level < recipe.levelReq;
                    ...
                    return (
                        <BouncyButton
                            ...
                            style={[
                                ...
                                isActive && [styles.nodeCardActive, { borderColor: herbColor, backgroundColor: herbColor + '11' }],
                            ...
                            <View style={[styles.nodeStats, isLevelLocked && { opacity: 0.4 }]}>
                                ...
                                <View style={styles.statPill}>
                                    ...
                                    <Text style={[styles.statValue, { color: palette.green }]}>
                                        {herbSkill.level >= 99 ? 'MAX' : `~${Math.ceil((nlvXP - herbSkill.xp) / recipe.xpPerTick)}`}
                                    </Text>
                                </View>
                                <View style={styles.statPill}>
                            ...
                            {!isLevelLocked && (
                                <View style={[
                                    ...
                                    <Text style={styles.trainButtonText}>
                                        {isActive ? 'Stop Brewing' : outOfMaterials ? 'No Materials' : 'Brew'}
                                    </Text>
                                </View>
                            )}
                            {isActive && activeTask && (
