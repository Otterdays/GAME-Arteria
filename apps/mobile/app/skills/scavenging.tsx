...
import { SCAVENGING_NODES, ScavengingNode } from '@/constants/scavenging';
...
export default function ScavengingScreen() {
    useIdleSoundscape('scavenging');
    const { palette } = useTheme();
    const { showFeedbackToast } = useFeedbackToast();
    const dispatch = useAppDispatch();
    const requestStartTask = useRequestStartTask();
    const insets = useSafeAreaInsets();
    const scavengingSkill = useAppSelector((s) => s.game.player.skills.scavenging);
    const activeTask = useAppSelector((s) => s.game.player.activeTask);

    const isScavenging = activeTask?.skillId === 'scavenging';
    const activeNodeId = isScavenging ? activeTask.actionId : null;
    const activeNode = SCAVENGING_NODES.find((n) => n.id === activeNodeId);

    const [popTrigger, setPopTrigger] = React.useState(0);
    const lastXp = React.useRef(scavengingSkill.xp);
    const lastGain = React.useRef(0);
    const shakeAnim = React.useRef(new Animated.Value(0)).current;

    React.useEffect(() => {
        if (scavengingSkill.xp > lastXp.current) {
            const gain = scavengingSkill.xp - lastXp.current;
            lastXp.current = scavengingSkill.xp;
    ...
    const clvXP = xpForLevel(scavengingSkill.level);
    const nlvXP = xpForLevel(scavengingSkill.level + 1);
    const xpIntoLevel = Math.max(0, Math.floor(scavengingSkill.xp - clvXP));
    const xpNeeded = Math.max(1, nlvXP - clvXP);
    const pct = scavengingSkill.level >= 99 ? 100 : Math.min(100, (xpIntoLevel / xpNeeded) * 100);

    const handleNodePress = (node: ScavengingNode) => {
        if (scavengingSkill.level < node.levelReq) {
            showFeedbackToast({
                type: 'locked',
                title: 'Locked',
                message: `Requires Scavenging Level ${node.levelReq}`,
            });
            return;
        }
    ...
    return (
        <Animated.View style={[styles.container, { paddingTop: insets.top }, { transform: [{ translateX: shakeX }] }]}>
            <Stack.Screen options={{ title: 'Scavenging', headerShown: false }} />
                ...
                <View style={styles.xpRow}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={styles.xpText}>
                            {scavengingSkill.level >= 99 ? '' : <AnimatedNumber value={xpIntoLevel} formatValue={(v) => formatNumber(v)} />}
                            {scavengingSkill.level >= 99 ? `${formatNumber(scavengingSkill.xp)} XP — MAX` : ` / ${formatNumber(xpNeeded)} XP`}
                        </Text>
                    </View>
            ...
            <ScrollView contentContainerStyle={styles.listContent}>
                {SCAVENGING_NODES.map((node) => {
                    const isLocked = scavengingSkill.level < node.levelReq;
                        ...
                        >
                            ...
                            <View style={[styles.nodeStats, isLocked && { opacity: 0.5 }]}>
                                ...
                                <View style={styles.statPill}>
                                    ...
                                    <Text style={[styles.statValue, { color: palette.green }]}>
                                        {scavengingSkill.level >= 99 ? 'MAX' : `~${Math.ceil((nlvXP - scavengingSkill.xp) / node.xpPerTick)}`}
                                    </Text>
                                </View>
                                <View style={styles.statPill}>
                            ...
                            {!isLocked && (
                                <View style={[styles.trainButton, isActive && styles.trainButtonActive]}>
                                    <Text style={styles.trainButtonText}>{isActive ? 'Stop Scavenging' : 'Scavenge'}</Text>
                                </View>
                            )}
                            {isActive && activeTask && (
                                <SmoothProgressBar partialTickMs={activeTask.partialTickMs} intervalMs={activeTask.intervalMs} fillColor={palette.skillScavenging} />
                            )}
                            {isLocked && (
