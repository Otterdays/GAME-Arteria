...
## 🧩 8. Android Studio after a fresh clone

If Android Studio shows **no run configuration** or a project without `settings.gradle` at the root, you almost always opened the wrong folder or have not generated native projects yet. **`android/` under `apps/mobile` is not in Git** (CNG — §1). Run `npx expo prebuild --platform android` from `apps/mobile` (or use `1_Run_Local_Android_Build.bat` / `2_Build_APK_Local.bat`), then **open `apps/mobile/android`** in Studio — not `apps/mobile/android/app`, and not an empty repo-root `android` folder.

**Full checklist:** **[ANDROID_STUDIO_ONBOARDING.md](ANDROID_STUDIO_ONBOARDING.md)**.
